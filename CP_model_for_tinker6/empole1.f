...
! qtw << end
      ...
      integer iax,iay,iaz
      integer kax,kay,kaz
      integer typei, typek  ! qtw
      real*8 tmp_e_qq       ! qtw
      real*8 tmp_e_qp       ! qtw
      real*8 tmp_dqdr(3)    ! qtw
      real*8 tmp_dpdr(3)    ! qtw
      real*8 tmp_tiqj(3)    ! qtw
      real*8 tmp_tjqi(3)    ! qtw
...
c
     ...
     &                   + rr7*gli(3)*psc7)
! qtw >> in - charge penetration correction
               if (ecp%use_chgpen) then
                 !! substract the old q-q, q-p energy
                 !e = e - gl(0)*rr1 - (ck*sc(3)-ci*sc(4))*rr3

                 !write(*,*)"typei =", type(ii)
                 !write(*,*)"typej =", type(kk)
                 !write(*,*)"n_vali =", ecp%n_val(type(ii))
                 !write(*,*)"n_valj =", ecp%n_val(type(kk))

                 typei = type(ii)
                 typek = type(kk)

                 call ecp_qp_form_1(ecp%n_val(typei), ecp%n_val(typek),
     &                            ecp%alp_jp(typei), ecp%alp_jp(typek),
     &                            ecp%bet_jp(typei), ecp%bet_jp(typek),
     &            xr, yr, zr,
     &            ci, di(1), di(2), di(3), ck, dk(1), dk(2), dk(3),
     &            tmp_e_qq, tmp_e_qp, tmp_dqdr, tmp_dpdr, tmp_tiqj,
     &            tmp_tjqi)

                 ! substract the old q-q energy
                 e = e - gl(0)*rr1
...
! qtw >> in - charge penetration correction
               if (ecp%use_chgpen) then
!                 ! substract the old q-q, q-p gradient
!                 ftm2(1) = ftm2(1) - ci*ck*rr3*xr
!     &                             - (-ck*di(1)*rr3) - ci*dk(1)*rr3
!     &                             - (ck*sc(3)-ci*sc(4))*rr5*xr
!                 ftm2(2) = ftm2(2) - ci*ck*rr3*yr
!     &                             - (-ck*di(2)*rr3) - ci*dk(2)*rr3
!     &                             - (ck*sc(3)-ci*sc(4))*rr5*yr
!                 ftm2(3) = ftm2(3) - ci*ck*rr3*zr
!     &                             - (-ck*di(3)*rr3) - ci*dk(3)*rr3
!     &                             - (ck*sc(3)-ci*sc(4))*rr5*zr

                 ! substract the old q-q gradient
                 ftm2(1) = ftm2(1) - ci*ck*rr3*xr
                 ftm2(2) = ftm2(2) - ci*ck*rr3*yr
                 ftm2(3) = ftm2(3) - ci*ck*rr3*zr

                 !! add the new q-q, q-p gradient
                 !ftm2(1) = ftm2(1) + ecp%dedr_ix
                 !ftm2(2) = ftm2(2) + ecp%dedr_iy
                 !ftm2(3) = ftm2(3) + ecp%dedr_iz

                 ! add the new q-q gradient
                 ftm2(1) = ftm2(1) + tmp_dqdr(1)
                 ftm2(2) = ftm2(2) + tmp_dqdr(2)
                 ftm2(3) = ftm2(3) + tmp_dqdr(3)

!                 write(*,*) "old: dr x = ", ci*ck*rr3*xr
!     &                                 + (-ck*di(1)*rr3) + ci*dk(1)*rr3
!     &                                 + (ck*sc(3)-ci*sc(4))*rr5*xr
!                 write(*,*) "old: dr y = ", ci*ck*rr3*yr
!     &                                 + (-ck*di(2)*rr3) + ci*dk(2)*rr3
!     &                                 + (ck*sc(3)-ci*sc(4))*rr5*yr
!                 write(*,*) "old: dr z = ", ci*ck*rr3*zr
!     &                                 + (-ck*di(3)*rr3) + ci*dk(3)*rr3
!     &                                 + (ck*sc(3)-ci*sc(4))*rr5*zr
                 ...
                 !write(*,*)
                 !write(*,*) "new: dr x = ", ecp%dqdr_ix
                 !write(*,*) "new: dr y = ", ecp%dqdr_iy
                 !write(*,*) "new: dr z = ", ecp%dqdr_iz
               endif
! qtw << end
c
...
!! qtw >> in - charge penetration correction
!               if (ecp%use_chgpen) then
!                 ! q-q interaction does not have torque
!                 !! substract the old q-q, q-p torque
!                 !!ttm2(1) = ttm2(1) - ( -ck*dixr(1) )
!                 !!ttm2(2) = ttm2(2) - ( -ck*dixr(2) )
!                 !!ttm2(3) = ttm2(3) - ( -ck*dixr(3) )
!                 !ttm2(1) = ttm2(1) + ck*dixr(1)
!                 !ttm2(2) = ttm2(2) + ck*dixr(2)
!                 !ttm2(3) = ttm2(3) + ck*dixr(3)
!                 !
!                 !ttm3(1) = ttm3(1) - ci*dkxr(1)
!                 !ttm3(2) = ttm3(2) - ci*dkxr(2)
!                 !ttm3(3) = ttm3(3) - ci*dkxr(3)
!                 !
!                 !! add the new q-q, q-p torque
!                 !ttm2(1) = ttm2(1) + ecp%t_ix
!                 !ttm2(2) = ttm2(2) + ecp%t_iy
!                 !ttm2(3) = ttm2(3) + ecp%t_iz
!                 !
!                 !ttm3(1) = ttm3(1) + ecp%t_jx
!                 !ttm3(2) = ttm3(2) + ecp%t_jy
!                 !ttm3(3) = ttm3(3) + ecp%t_jz
!               end if
!! qtw << end
c
...
! qtw >> in - charge penetration correction
               if (ecp%use_chgpen) then
                 ...
                 !write(*,*) "replica: old: dr x = ", ci*ck*rr3*xr
                 !write(*,*) "replica: old: dr y = ", ci*ck*rr3*yr
                 !write(*,*) "replica: old: dr z = ", ci*ck*rr3*zr

                 !write(*,*)
                 !write(*,*) "replicat: new: dr x = ", ecp%dqdr_ix
                 !write(*,*) "replicat: new: dr y = ", ecp%dqdr_iy
                 !write(*,*) "replicat: new: dr z = ", ecp%dqdr_iz
               endif
! qtw << end
c
c     get the induced force components
...
! qtw >> in - charge penetration correction
               if (ecp%use_chgpen) then
                 ! For Ewald part, maybe it is better to overide gl
                 ! directly
     ...
     &            tmp_tjqi)
                 ...
                 ! substract the old unscreened q-q energy
                 ! and add the new unscreened CP q-q energy
                 !e = e - rr1*gl(0)*mscale(kk) + tmp_e_qq*mscale(kk)
                 e = e + (tmp_e_qq - rr1*gl(0))*mscale(kk)

                 ! do the same for erl
                 erl = erl - rr1*gl(0) + tmp_e_qq
               end if
! qtw << end
               !e = e - (1.0d0-mscale(kk))*erl
               e = e - (1.0d0-mscale(kk))*erl_no_cp
               ei = ei - erli
               e = f * e
               ei = f * ei
               em = em + e
...
! qtw >> in - charge penetration correction
               if (ecp%use_chgpen) then
!old-wrong                 ! substract the old q-q gradient
!old-wrong                 ftm2(1) = ftm2(1) - bn(1)*gl(0)*xr
!old-wrong                 ftm2(2) = ftm2(2) - bn(1)*gl(0)*yr
!old-wrong                 ftm2(3) = ftm2(3) - bn(1)*gl(0)*zr
!old-wrong
!old-wrong                 ! erfc0  = erfc(ralpha)
!old-wrong                 ! erfc0' = alsq2n*exp(-ralpha**2)
!old-wrong                 tmp_alsq2n = 0.0D0
!old-wrong                 if (aewald .gt. 0.0d0)
!old-wrong     &              tmp_alsq2n = 1.0d0 / (sqrtpi*aewald)
!old-wrong                 tmp_alsq2n = tmp_alsq2n * 2.0D0 * aewald**2
!old-wrong                 tmp_erfc0 = erfc(aewald * r)
!old-wrong                 tmp_erfc0d = tmp_alsq2n*exp(-(aewald*r)**2)
!old-wrong
!old-wrong                 ftm2(1) = ftm2(1) + tmp_dqdr(1)*tmp_erfc0
!old-wrong     &                             + tmp_e_qq*tmp_erfc0d*rr1*xr
!old-wrong
!old-wrong                 ftm2(2) = ftm2(2) + tmp_dqdr(2)*tmp_erfc0
!old-wrong     &                             + tmp_e_qq*tmp_erfc0d*rr1*yr  
!old-wrong
!old-wrong                 ftm2(3) = ftm2(3) + tmp_dqdr(3)*tmp_erfc0
!old-wrong     &                             + tmp_e_qq*tmp_erfc0d*rr1*zr  

                 !! substract the old q-q gradient
                 !ftm2r(1) = ftm2r(1) - rr3*gl(0)*xr
                 !ftm2r(2) = ftm2r(2) - rr3*gl(0)*yr
                 !ftm2r(3) = ftm2r(3) - rr3*gl(0)*zr

                 !! add the new q-q gradient
                 !ftm2r(1) = ftm2r(1) + tmp_dqdr(1)
                 !ftm2r(2) = ftm2r(2) + tmp_dqdr(2)
                 !ftm2r(3) = ftm2r(3) + tmp_dqdr(3)

                 ! substract the old q-q gradient,
                 !   and add the new q-q gradient
                 ftm2(1) = ftm2(1) + (tmp_dqdr(1) - rr3*gl(0)*xr) 
     &                             * mscale(kk)
                 ftm2(2) = ftm2(2) + (tmp_dqdr(2) - rr3*gl(0)*yr)
     &                             * mscale(kk)
                 ftm2(3) = ftm2(3) + (tmp_dqdr(3) - rr3*gl(0)*zr)
     &                             * mscale(kk)
               endif
! qtw << end
c
...
!$OMP& gfr,gfri,gti,gtri,dorl,dorli,tmp_e_qq,tmp_e_qp,tmp_dqdr,
!$OMP& tmp_dpdr,tmp_tiqj,tmp_tjqi,typei,typek,erl_no_cp)
...
! qtw << end
               ...
               erl_no_cp = erl_no_cp * (1.0d0-mscale(kk))
               erli = 0.5d0*(rr3*(gli(1)+gli(6))*psc3
     &                   + rr5*(gli(2)+gli(7))*psc5
     &                   + rr7*gli(3)*psc7)
               e = e - erl_no_cp
...
!old-wrong ! qtw >> in - charge penetration correction
!old-wrong                   if (ecp%use_chgpen) then
!old-wrong                     !! substract the old q-q gradient
!old-wrong                     !ftm2r(1) = ftm2r(1) - rr3*gl(0)*xr
!old-wrong                     !ftm2r(2) = ftm2r(2) - rr3*gl(0)*yr
!old-wrong                     !ftm2r(3) = ftm2r(3) - rr3*gl(0)*zr
!old-wrong 
!old-wrong                     !! add the new q-q gradient
!old-wrong                     !ftm2r(1) = ftm2r(1) + tmp_dqdr(1)
!old-wrong                     !ftm2r(2) = ftm2r(2) + tmp_dqdr(2)
!old-wrong                     !ftm2r(3) = ftm2r(3) + tmp_dqdr(3)
!old-wrong                   endif
!old-wrong ! qtw << end
               end if
c
