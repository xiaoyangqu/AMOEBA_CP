...
! qtw >> in - charge penetration correction
               if (ecp%use_chgpen) then
                 ...
                 !write(iout,*)"e3c_ds:ii =", ii
                 !write(iout,*)"e3c_ds:jj =", kk
                 !write(iout,*)"e3c_ds:typei =", typei
                 !write(iout,*)"e3c_ds:typej =", typek
                 !write(iout,*)"e3c_ds:n_vali =", ecp%n_val(type(ii))
                 !write(iout,*)"e3c_ds:n_valj =", ecp%n_val(type(kk))
     ...
     &            tmp_tjqi)
                 ...
                 !e = e - rr1*gl(0) + tmp_e_qq
                 e = e - rr1*gl(0)*mscale(kk) + tmp_e_qq*mscale(kk)
   
                 ! do the same for efix
                 efix = efix - rr1*gl(0) + tmp_e_qq
               end if
! qtw << end
...
c
               ...
               e = e - efix_no_cp*(1.0d0-mscale(kk))
               ei = ei - eifix
               e = f * e
               ei = 0.5d0 * f * ei
c
...
! qtw >> in - charge penetration correction
                  if (ecp%use_chgpen) then
                    ...
                    !write(iout,*)"e3c_rs:ii =", ii
                    !write(iout,*)"e3c_rs:jj =", kk
                    !write(iout,*)"e3c_rs:typei =", typei
                    !write(iout,*)"e3c_rs:typej =", typek
                    !write(iout,*)"e3c_rs:n_vali =", ecp%n_val(type(ii))
                    !write(iout,*)"e3c_rs:n_valj =", ecp%n_val(type(kk))
...
!$OMP& bn,sc,gl,sci,gli,tmp_e_qq,tmp_e_qp,typei,typek,efix_no_cp)
...
! qtw >> in - charge penetration correction
               if (ecp%use_chgpen) then
                 ...
                 !write(iout,*) "e3d:ii =", ii
                 !write(iout,*) "e3d:jj =", kk
                 !write(iout,*) "e3d:typei =", typei
                 !write(iout,*) "e3d:typej =", typek
                 !write(iout,*)"e3d:n_vali =", ecp%n_val(type(ii))
                 !write(iout,*)"e3d:n_valj =", ecp%n_val(type(kk))
...
c
               !e = e - efix*(1.0d0-mscale(kk))
               e = e - efix_no_cp*(1.0d0-mscale(kk))  !qtw
               ei = ei - eifix
c
c     increment the overall multipole and polarization energies
c
