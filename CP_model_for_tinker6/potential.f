...
c
   ...
   20    format (/,' The TINKER Electrostatic Potential Facility Can :',
     ...
     &           /,4x,'(7) Compare Chgpen Potential to a Target Grid',
     &           /,4x,'(8) Fit Chgpen Alpha to a Target Grid')
         !do while (mode.lt.1 .or. mode.gt.6)
         do while (mode.lt.1 .or. mode.gt.8)
            mode = 0
            write (iout,30)
...
c
      do i = 1, nkey
         ...
         else if (keyword(1:17) .eq. 'POTENTIAL-CHGPEN ') then
            read (string,*,err=10,end=10)  ichgpen
            call upcase(ichgpen)
         end if
   10    continue
      end do
      write(*,'(" OFFSET: ", f13.3)') roffset
      if (ichgpen == 'ON') then
         write(*,'(" POTENTIAL-CHGPEN is ON! ")') 
      end if
c
c     perform dynamic allocation of some local arrays
c
...
c
      do i = 1, n
         ...
         if (anum .eq. 54)  rad(i) = 2.16d0
         if (ichgpen == 'ON') then
            ! overide the rad value when build grid to fit chgpen prm
            if (anum .eq. 1)  rad(i) = 1.30d0
            if (anum .eq. 6)  rad(i) = 2.10d0
            if (anum .eq. 7)  rad(i) = 1.30d0
            if (anum .eq. 8)  rad(i) = 1.30d0
            if (anum .eq. 9)  rad(i) = 1.30d0
            if (anum .eq. 15)  rad(i) = 1.80d0
            if (anum .eq. 16)  rad(i) = 1.70d0
            if (anum .eq. 17)  rad(i) = 1.60d0
            if (anum .eq. 35)  rad(i) = 1.60d0
         end if
         rad(i) = rad(i) + roffset
         rad2(i) = rad(i)**2
      end do
...
c
         ...
         rr3 = rr1 / r2
         rr5 = 3.0d0 * rr3 / r2
         if (potcp%dochgpen) then
            typekk = type(kk)
            nkk = real(ecp%n_val(typekk))
            tmp_pot = (nkk - ck)
     &               *(1.0D0 - exp(-ecp%alp_jp(typekk)*r))
            !write(*,*) "nkk", nkk
            !write(*,*) "alp", ecp%alp_jp(typekk)
!potential without switching function
!            e = (nkk - tmp_pot)*rr1 - scd*rr3 + scq*rr5

            e_switch = 0.0D0
            dr_switch = 0.0D0
            call chgpen_switch_func(r, ecp%switch_lo, ecp%switch_hi,
     &                              e_switch, dr_switch)
!potential with switching function
            e = (nkk - tmp_pot)*rr1*e_switch
     &        + ck*rr1*(1.0D0 - e_switch)
     &        - scd*rr3 + scq*rr5

!test            tmp_pot = nkk - ck
!test     &               *(1.0D0 - exp(-ecp%alp_jp(typekk)*r))
!test            !write(*,*) "nkk", nkk
!test            !write(*,*) "alp", ecp%alp_jp(typekk)
!test            e = (nkk - tmp_pot)*rr1 - scd*rr3 + scq*rr5
         else
